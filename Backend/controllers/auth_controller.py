...
def get_nearby_farmers():
    """Returns a list of active farmers for the customer dashboard."""
    try:
        farmers = User.query.filter_by(role='farmer', is_active=True).limit(20).all()
        result = []
        for f in farmers:
            result.append({
                'id': f.id,
                ...
                'crops_category': f.crops_category,
                'rating': round(f.rating, 1) if f.rating else 0.0,
                'rating_count': f.rating_count,
            })
        return jsonify({'farmers': result}), 200
    except Exception as e:
        return jsonify({'farmers': [], 'error': str(e)}), 200
